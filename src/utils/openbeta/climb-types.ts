// https://github.com/OpenBeta/openbeta-graphql/blob/develop/src/db/ClimbTypes.ts

import type { Point } from '@turf/helpers';
import type { GradeScalesTypes } from '@openbeta/sandbag';

import type { ChangeRecordMetadataType } from './change-log-type';
import type { GradeContexts } from './grade-utils';
...
type UpdatableClimbFieldsType = Pick<
  ClimbType,
  'fa' | 'name' | 'type' | 'gradeContext' | 'grades' | 'content'
>;
/**
 * Minimum required fields when adding a new climb or boulder problem
 */
export type ClimbChangeDocType = Partial<UpdatableClimbFieldsType> & {
  _id: string;
  metadata: Pick<
    ClimbType['metadata'],
    'areaRef' | 'left_right_index' | 'lnglat'
  >;
} & { _change: ChangeRecordMetadataType };
